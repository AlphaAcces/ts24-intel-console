...
const DetailRow: React.FC<{ label: string; value: string | undefined; fallback?: string }> = ({ label, value, fallback }) => (
    <div className="py-2 grid grid-cols-3 gap-4">
        ...
        <dd className="text-sm text-gray-200 col-span-2 font-mono">{value || fallback || 'N/A'}</dd>
    </div>
);
      ...
      >
        ...
        <div className="p-6">
            <dl className="divide-y divide-border-dark/50">
                <DetailRow label={t('companies:modal.fields.cvr')} value={company.cvr} fallback={t('common:naShort')} />
                <DetailRow label={t('companies:modal.fields.status')} value={getStatusLabel(company.status)} fallback={t('common:naShort')} />
                <DetailRow label={t('companies:modal.fields.established')} value={company.established} fallback={t('common:naShort')} />
                <DetailRow label={t('companies:modal.fields.industryCode')} value={company.industryCode} fallback={t('common:naShort')} />
                <DetailRow label={t('companies:modal.fields.director')} value={company.director} fallback={t('common:naShort')} />
                <DetailRow label={t('companies:modal.fields.owner')} value={company.owner} fallback={t('common:naShort')} />
                <DetailRow label={t('companies:modal.fields.auditor')} value={company.auditor} fallback={t('common:naShort')} />
            </dl>
